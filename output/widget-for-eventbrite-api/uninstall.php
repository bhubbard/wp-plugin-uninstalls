...
delete_site_option('widget-for-eventbrite-api-settings');
delete_option('widget-for-eventbrite-api-version');
delete_site_option('widget-for-eventbrite-api-version');
delete_option('widget-for-eventbrite-api-settings-api');
delete_site_option('widget-for-eventbrite-api-settings-api');
delete_option('wfea_transients');
delete_site_option('wfea_transients');
...
// Delete Transients
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_% _bak', '_site_transient_% _bak' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
}
delete_transient('_x_wfea_transients');
delete_site_transient('_x_wfea_transients');
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_%_bak', '_site_transient_%_bak' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
