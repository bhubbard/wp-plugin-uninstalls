...
// Delete Options
delete_option('ansar_demo_installed');
delete_site_option('ansar_demo_installed');
global $wpdb;
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_customizer_import' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
