...
delete_site_option('cntctfrmmlt_options');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'cntctfrmmlt_options_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('cntctfrmmltpr_options_main');
delete_site_option('cntctfrmmltpr_options_main');
delete_option('cntctfrmmlt_options_1');
delete_site_option('cntctfrmmlt_options_1');
delete_option('cntctfrmmlt_options_main');
delete_site_option('cntctfrmmlt_options_main');
