...
delete_option('xoo_tracking_consent_easy-login-woocommerce');
delete_site_option('xoo_tracking_consent_easy-login-woocommerce');
...
delete_option('xoo-el-av-options');
delete_site_option('xoo-el-av-options');
global $wpdb;
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_allow_old_layout' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
