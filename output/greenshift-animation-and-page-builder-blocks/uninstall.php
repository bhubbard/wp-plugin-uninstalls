...
delete_option('gspb_block_js');
delete_site_option('gspb_block_js');
delete_option('gspb_edd_licenses');
delete_site_option('gspb_edd_licenses');
delete_option('greenshift_global_classes');
delete_site_option('greenshift_global_classes');
delete_option('gspb_css_save');
delete_site_option('gspb_css_save');
delete_option('gspb_stylebook_id');
delete_site_option('gspb_stylebook_id');
delete_option('greenshift_theme_options');
delete_site_option('greenshift_theme_options');

// Delete Transients
delete_transient('gspb-dynamic-icons-render');
delete_site_transient('gspb-dynamic-icons-render');
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_gs_countdown_sale%', '_site_transient_gs_countdown_sale%' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
}
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_gs_countdown_sale_hours%', '_site_transient_gs_countdown_sale_hours%' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
}

// Clear Cron Jobs
wp_clear_scheduled_hook('greenshift_check_cron_hook');
...
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = %s", '_gspb_post_css' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key = %s", '_gspb_post_css' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->termmeta} WHERE meta_key = %s", '_gspb_post_css' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->commentmeta} WHERE meta_key = %s", '_gspb_post_css' ) );
