...
DELETE FROM wp_options WHERE option_name IN ('recently_activated', 'woocommerce_tax_display_cart');
...
-- Delete Metadata
DELETE FROM wp_postmeta WHERE meta_key IN ('_raq_request');
DELETE FROM wp_usermeta WHERE meta_key IN ('_raq_request');
DELETE FROM wp_termmeta WHERE meta_key IN ('_raq_request');
DELETE FROM wp_commentmeta WHERE meta_key IN ('_raq_request');
