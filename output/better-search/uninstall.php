<?php

// If uninstall not called from WordPress, then exit.
if ( ! defined( 'WP_UNINSTALL_PLUGIN' ) ) {
	exit;
}

// Delete Options
delete_option('bsearch_db_version');
delete_site_option('bsearch_db_version');
delete_option('bsearch_settings');
delete_site_option('bsearch_settings');
delete_option('bsearch_show_wizard');
delete_site_option('bsearch_show_wizard');
delete_option('bsearch_wizard_notice_dismissed');
delete_site_option('bsearch_wizard_notice_dismissed');
delete_option('wz_posts_custom_tables_ready');
delete_site_option('wz_posts_custom_tables_ready');
delete_option('ald_bsearch_settings');
delete_site_option('ald_bsearch_settings');
delete_option('better_search_selected_sites');
delete_site_option('better_search_selected_sites');
delete_option('bsearch_wizard_completed');
delete_site_option('bsearch_wizard_completed');
delete_option('bsearch_wizard_completed_date');
delete_site_option('bsearch_wizard_completed_date');
delete_option('bsearch_wizard_current_step');
delete_site_option('bsearch_wizard_current_step');
delete_option('fs_debug_mode');
delete_site_option('fs_debug_mode');
delete_option('_transient_timeout_fs_snooze_period');
delete_site_option('_transient_timeout_fs_snooze_period');
delete_option('_site_transient_timeout_fs_snooze_period');
delete_site_option('_site_transient_timeout_fs_snooze_period');
delete_option('active_sitewide_plugins');
delete_site_option('active_sitewide_plugins');
delete_option('fs_storage_logger');
delete_site_option('fs_storage_logger');
delete_option('fs_active_plugins');
delete_site_option('fs_active_plugins');

// Delete Transients
delete_transient('bsearch_deactivated_notice');
delete_site_transient('bsearch_deactivated_notice');
delete_transient('bsearch_show_wizard_activation_redirect');
delete_site_transient('bsearch_show_wizard_activation_redirect');
delete_transient('bsearch_reindex_state');
delete_site_transient('bsearch_reindex_state');
delete_transient('bsearch_deactivated_notice_id');
delete_site_transient('bsearch_deactivated_notice_id');
delete_transient('bsearch_reindex_scheduled');
delete_site_transient('bsearch_reindex_scheduled');
delete_transient('fs_snooze_period');
delete_site_transient('fs_snooze_period');
delete_transient('update_plugins');
delete_site_transient('update_plugins');
delete_transient('_fs_api_connection_retry_counter');
delete_site_transient('_fs_api_connection_retry_counter');
delete_transient('update_themes');
delete_site_transient('update_themes');

// Clear Cron Jobs
wp_clear_scheduled_hook('fs_debug_turn_off_logging_hook');

// Clear Metadata
global $wpdb;
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = %s", '_video_thumbnail' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key = %s", '_video_thumbnail' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->termmeta} WHERE meta_key = %s", '_video_thumbnail' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->commentmeta} WHERE meta_key = %s", '_video_thumbnail' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = %s", '_wp_attachment_image_alt' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key = %s", '_wp_attachment_image_alt' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->termmeta} WHERE meta_key = %s", '_wp_attachment_image_alt' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->commentmeta} WHERE meta_key = %s", '_wp_attachment_image_alt' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = %s", '_seopress_robots_primary_cat' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->usermeta} WHERE meta_key = %s", '_seopress_robots_primary_cat' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->termmeta} WHERE meta_key = %s", '_seopress_robots_primary_cat' ) );
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->commentmeta} WHERE meta_key = %s", '_seopress_robots_primary_cat' ) );

