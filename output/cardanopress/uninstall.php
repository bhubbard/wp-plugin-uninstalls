...
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_%activating', '_site_transient_%activating' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
}
