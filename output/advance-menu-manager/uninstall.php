...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'amm_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('amm_post_perpage_default');
delete_site_option('amm_post_perpage_default');
delete_option('amm_post_perpage_option');
delete_site_option('amm_post_perpage_option');
delete_option('nav_menu_options');
delete_site_option('nav_menu_options');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'menu_lock_status_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'amm_user_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('dsamm_where_hear_about_us');
delete_site_option('dsamm_where_hear_about_us');
delete_option('dsamm_data_submited_in_sendiblue');
delete_site_option('dsamm_data_submited_in_sendiblue');
