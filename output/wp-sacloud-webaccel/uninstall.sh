#!/bin/bash
# WP-CLI Uninstall Script

# Delete Options
wp option delete 'sacloud-webaccel-options'

# Direct DB Queries (Fallback)
wp db query "DELETE FROM wp_postmeta WHERE meta_key = 'sacloud_nocache'"
wp db query "DELETE FROM wp_usermeta WHERE meta_key = 'sacloud_nocache'"
wp db query "DELETE FROM wp_termmeta WHERE meta_key = 'sacloud_nocache'"
wp db query "DELETE FROM wp_commentmeta WHERE meta_key = 'sacloud_nocache'"
wp db query "DELETE FROM wp_postmeta WHERE meta_key = '_wp_attachment_backup_sizes'"
wp db query "DELETE FROM wp_usermeta WHERE meta_key = '_wp_attachment_backup_sizes'"
wp db query "DELETE FROM wp_termmeta WHERE meta_key = '_wp_attachment_backup_sizes'"
wp db query "DELETE FROM wp_commentmeta WHERE meta_key = '_wp_attachment_backup_sizes'"
