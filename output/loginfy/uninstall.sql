...
DELETE FROM wp_options WHERE option_name IN ('jlt_loginfy_sheet_promo_data', 'jlt_loginfy_sheet_promo_data_hash', 'jltwp_adminify_login', 'admin_email_lifespan', 'active_sitewide_plugins', 'jlt_loginfy_activation_time', 'fs_debug_mode', '_transient_timeout_fs_snooze_period', '_site_transient_timeout_fs_snooze_period', 'fs_storage_logger', 'fs_active_plugins', '_welcome_screen_activation_redirect', 'fs_snooze_period', 'update_plugins', '_fs_api_connection_retry_counter', 'update_themes');
DELETE FROM wp_options WHERE option_name LIKE '%_is_synced';
DELETE FROM wp_options WHERE option_name LIKE '%_allow_tracking';
DELETE FROM wp_options WHERE option_name LIKE 'jlt_loginfy-plugin-info-%';
...
DELETE FROM wp_postmeta WHERE meta_key LIKE '_loginfy_errors_%';
DELETE FROM wp_usermeta WHERE meta_key LIKE '_loginfy_errors_%';
DELETE FROM wp_termmeta WHERE meta_key LIKE '_loginfy_errors_%';
DELETE FROM wp_commentmeta WHERE meta_key LIKE '_loginfy_errors_%';
