...
DELETE FROM wp_options WHERE option_name IN ('opalservice_settings', 'elementor_cpt_support', 'opalservice_version', 'opalservice_version_upgraded_from', 'opalservice_setup', '_opalservice_installed', '_opalservice_activation_redirect');

-- Delete Metadata
DELETE FROM wp_postmeta WHERE meta_key IN ('phone', '_pending_to_use_', '_wp_page_template', 'category-image-id');
DELETE FROM wp_usermeta WHERE meta_key IN ('phone', '_pending_to_use_', '_wp_page_template', 'category-image-id');
DELETE FROM wp_termmeta WHERE meta_key IN ('phone', '_pending_to_use_', '_wp_page_template', 'category-image-id');
DELETE FROM wp_commentmeta WHERE meta_key IN ('phone', '_pending_to_use_', '_wp_page_template', 'category-image-id');
