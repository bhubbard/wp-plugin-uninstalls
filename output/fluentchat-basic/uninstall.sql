-- WordPress Plugin Uninstall SQL Script

-- Delete Options & Transients
DELETE FROM wp_options WHERE option_name LIKE '%ActivatedTime';
DELETE FROM wp_options WHERE option_name LIKE '%Settings';
DELETE FROM wp_options WHERE option_name LIKE '%Styling';
DELETE FROM wp_options WHERE option_name LIKE '%Online';
DELETE FROM wp_options WHERE option_name LIKE '%Pestered';
DELETE FROM wp_options WHERE option_name LIKE '%ValidKey';

-- Delete Metadata
DELETE FROM wp_postmeta WHERE meta_key IN ('first_name', 'last_name');
DELETE FROM wp_usermeta WHERE meta_key IN ('first_name', 'last_name');
DELETE FROM wp_termmeta WHERE meta_key IN ('first_name', 'last_name');
DELETE FROM wp_commentmeta WHERE meta_key IN ('first_name', 'last_name');

