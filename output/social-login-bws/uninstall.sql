...
DELETE FROM wp_options WHERE option_name IN ('bstwbsftwppdtplgns_options', 'recently_activated', 'active_sitewide_plugins', 'scllgn_options', 'update_plugins', 'bws_plugins_update');
...
DELETE FROM wp_postmeta WHERE meta_key LIKE '%_login';
DELETE FROM wp_usermeta WHERE meta_key LIKE '%_login';
DELETE FROM wp_termmeta WHERE meta_key LIKE '%_login';
DELETE FROM wp_commentmeta WHERE meta_key LIKE '%_login';
