...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_notify_list' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_notify_init_list' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_limitby_caps' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_incoming_email' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('emd_inc_email_apps');
delete_site_option('emd_inc_email_apps');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_inline_ent' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('emd_inline_entity_apps');
delete_site_option('emd_inline_entity_apps');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_woocommerce' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_woocommerce_forms_list' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_edd' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_edd_forms_list' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_has_mailchimp' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
...
delete_option('wp_ticket_com_ent_map_list');
delete_site_option('wp_ticket_com_ent_map_list');
delete_option('wp_ticket_com_tools');
delete_site_option('wp_ticket_com_tools');
global $wpdb;
...
delete_option('wp_ticket_com_access_views');
delete_site_option('wp_ticket_com_access_views');
delete_option('wp_ticket_com_tax_settings');
delete_site_option('wp_ticket_com_tax_settings');
delete_option('wp_ticket_com_tracking_optin');
delete_site_option('wp_ticket_com_tracking_optin');
delete_option('wp_ticket_com_show_rateme_plugin_min');
delete_site_option('wp_ticket_com_show_rateme_plugin_min');
delete_option('wp_ticket_com_ent_list');
delete_site_option('wp_ticket_com_ent_list');
delete_option('wp_ticket_com_activation_date');
delete_site_option('wp_ticket_com_activation_date');
delete_option('wp_ticket_com_tax_list');
delete_site_option('wp_ticket_com_tax_list');
delete_option('wp_ticket_com_rel_list');
delete_site_option('wp_ticket_com_rel_list');
delete_option('wp_ticket_com_shc_list');
delete_site_option('wp_ticket_com_shc_list');
delete_option('wp_ticket_com_misc_settings');
delete_site_option('wp_ticket_com_misc_settings');
global $wpdb;
...
delete_option('wp_ticket_com_attr_list');
delete_site_option('wp_ticket_com_attr_list');
delete_option('wp_ticket_com_setup_pages_list');
delete_site_option('wp_ticket_com_setup_pages_list');
