...
delete_site_option('disabled_post_ids');
delete_option('lightbox_photoswipe_use_cache');
delete_site_option('lightbox_photoswipe_use_cache');
delete_option('lightbox_photoswipe_add_lazyloading');
delete_site_option('lightbox_photoswipe_add_lazyloading');
global $wpdb;
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'lightbox_photoswipe_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
