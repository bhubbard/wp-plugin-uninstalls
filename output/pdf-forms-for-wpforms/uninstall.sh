#!/bin/bash
# WP-CLI Uninstall Script

# Delete Options
wp option delete 'recently_activated'
wp option delete 'wpforms_settings'
wp option delete 'wpcf7'
wp option delete 'pdf-forms-for-woocommerce-settings-pdf-ninja-api-key'

# Delete Transients
wp transient delete 'update_plugins'
wp transient delete 'pdf_forms_for_wpforms_pdfninja_key_failure'

# Clear Cron Jobs
wp cron event delete 'pdf_forms_for_wpforms_cron'

# Direct DB Queries (Fallback)
wp db query "DELETE FROM wp_postmeta WHERE meta_key LIKE 'tgmpa_dismissed_notice_%'"
wp db query "DELETE FROM wp_usermeta WHERE meta_key LIKE 'tgmpa_dismissed_notice_%'"
wp db query "DELETE FROM wp_termmeta WHERE meta_key LIKE 'tgmpa_dismissed_notice_%'"
wp db query "DELETE FROM wp_commentmeta WHERE meta_key LIKE 'tgmpa_dismissed_notice_%'"
wp db query "DELETE FROM wp_postmeta WHERE meta_key LIKE 'pdf-forms-for-wpforms-%'"
wp db query "DELETE FROM wp_usermeta WHERE meta_key LIKE 'pdf-forms-for-wpforms-%'"
wp db query "DELETE FROM wp_termmeta WHERE meta_key LIKE 'pdf-forms-for-wpforms-%'"
wp db query "DELETE FROM wp_commentmeta WHERE meta_key LIKE 'pdf-forms-for-wpforms-%'"
