...
// Delete Options
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'ajaxhc_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('ajaxhc_use_rapid_incrementer');
delete_site_option('ajaxhc_use_rapid_incrementer');
delete_option('ajaxhc_dont_count_admins');
delete_site_option('ajaxhc_dont_count_admins');
