...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'fca_eoi_api_aweber_credentials_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('easy_opt_in_oath_infor_settings');
delete_site_option('easy_opt_in_oath_infor_settings');
