...
delete_site_option('logincust_setting');
...
delete_site_option('logincustomizer_review_dismiss');
delete_option('logincustomizer_active_time');
delete_site_option('logincustomizer_active_time');
...
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_review_flag' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", '%_translate_flag' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
...
delete_option('themeisle_sdk_active_notification');
delete_site_option('themeisle_sdk_active_notification');
global $wpdb;
...
global $wpdb;
$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s", '_transient_%versions', '_site_transient_%versions' ) );
foreach ( $transients as $transient ) {
	delete_option( $transient );
}
global $wpdb;
