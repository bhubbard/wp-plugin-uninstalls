...
// Delete Options
delete_option('ASSI_PLUGIN_BLOG_ACCESS_TOKEN');
delete_site_option('ASSI_PLUGIN_BLOG_ACCESS_TOKEN');
delete_option('ASSI_PLUGIN_ACCOUNT');
delete_site_option('ASSI_PLUGIN_ACCOUNT');
delete_option('ASSI_PLUGIN_GLOBAL_ALERTS');
delete_site_option('ASSI_PLUGIN_GLOBAL_ALERTS');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'ASSI_PLUGIN_BLOG_USER_OPTION_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('ASSI_PLUGIN_CONNECTION_TO_SERVER');
delete_site_option('ASSI_PLUGIN_CONNECTION_TO_SERVER');
delete_option('ASSI_PLUGIN_CUSTOM_AI_CONNECTION');
delete_site_option('ASSI_PLUGIN_CUSTOM_AI_CONNECTION');
global $wpdb;
$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name FROM {$wpdb->options} WHERE option_name LIKE %s", 'ASSI_PLUGIN_CUSTOM_STYLES_%' ) );
foreach ( $options as $opt ) {
	delete_option( $opt );
	delete_site_option( $opt );
}
delete_option('ASSI_PLUGIN_VERSION');
delete_site_option('ASSI_PLUGIN_VERSION');
