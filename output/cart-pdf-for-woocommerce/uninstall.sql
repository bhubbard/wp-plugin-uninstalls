...
DELETE FROM wp_options WHERE option_name IN ('yeepdf_woocommerce_cart_setup', 'ninja_forms_addons_feed', 'yeekit_addons');
...
-- Delete Metadata
DELETE FROM wp_postmeta WHERE meta_key IN ('data_email');
DELETE FROM wp_usermeta WHERE meta_key IN ('data_email');
DELETE FROM wp_termmeta WHERE meta_key IN ('data_email');
DELETE FROM wp_commentmeta WHERE meta_key IN ('data_email');
