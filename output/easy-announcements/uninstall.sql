...
DELETE FROM wp_options WHERE option_name IN ('easy_announcements', 'active_sitewide_plugins', 'acf_first_activated_version', 'acf_version', 'acf_escaped_html_notice_dismissed', 'cptui_post_types', 'cptui_taxonomies', 'acf_escaped_html_log', 'acf_will_escape_html_log', 'acf_deactivated_notice_id');
DELETE FROM wp_options WHERE option_name LIKE 'acf_network_upgrade_needed_%';
DELETE FROM wp_options WHERE option_name LIKE 'easy_announcements_%';
